...
## How the domain-based DNS split decides

**It is not runtime detection — it is a community-maintained static domain list plus
dnsmasq suffix matching.**

In `dnsmasq` (127.0.0.1:53):

- by default every name goes to `127.0.0.1#5353` (`unbound`, upstream over TCP via the
  proxy, anti-poisoning);
- names **present in the China domain list** go to `--direct-dns` (default `223.5.5.5`)
  directly.

The list is generated by `redsocks-refresh-domains` from
[felixonmars/dnsmasq-china-list](https://github.com/felixonmars/dnsmasq-china-list)
(`accelerated-domains.china.conf` + `apple.china.conf`, ~110k entries) plus a built-in
`.cn` fallback, written to `/etc/dnsmasq.d/china-domains.conf`:
...
Matching is **by domain suffix** (`server=/baidu.com/` matches `baidu.com` and its
subdomains, not `notbaidu.com`); with multiple matches the **longest (most specific)**
wins. Anything unmatched goes to the proxied DNS.

> ⚠️ **Domain split ≠ traffic split** — they are independent:

| Layer | Decides based on | What it determines |
| --- | --- | --- |
| **DNS** | domain list (static, suffix match) | domestic DNS directly vs. proxied DNS (anti-poisoning) |
| **TCP** | destination **IP** in `chnroute.txt` (APNIC/RIR data) | direct vs. through the proxy |

In short: **the domain list only picks which DNS to query; the destination IP decides
whether traffic is proxied.** An unlisted domestic domain is resolved via the proxy,
but is still routed direct once it resolves to a China CDN IP (just one extra DNS hop).

**Custom domains**: add `server=/your.domain/223.5.5.5` in a separate file under
`/etc/dnsmasq.d/` and `systemctl restart dnsmasq`. Do not edit `china-domains.conf`
directly — `redsocks-refresh-domains` overwrites it.
